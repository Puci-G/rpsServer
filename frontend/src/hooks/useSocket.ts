...
export const useSocket = () => {
  ...
  useEffect(() => {
   ...
   newSocket.on('loginError', (e:LoginError)=> setLoginErr(e.message));
    ...
    newSocket.on('queueJoined', (data: { position: number; queueSize: number }) => {
      setQueueState({
        inQueue: true,
    ...
    newSocket.on('gameFound', (data: { gameId: string; opponent: string; newBalance: number }) => {
      setGameState(prev => ({
        ...prev,
        gameId: data.gameId,
    ...
    newSocket.on('roundStart', (data: { round: number; player1Score: number; player2Score: number; timer: number }) => {
      setGameState(prev => ({
        ...prev,
    ...
    newSocket.on('roundResult', (data: RoundResultData) => {
      ...
      const result = validResults.includes(data.result as "win" | "lose" | "tie")
